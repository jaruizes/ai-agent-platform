...
Credentials are never stored in the ProcessServiceDefinition. Process Platform
mounts `.secrets` read-only and reads `/run/secrets/google/google-token.json`.
That file contains the OAuth client id/secret and refresh token generated by the
same Google Workspace auth flow used by Agent Platform. Process Platform exchanges
the refresh token for short-lived access tokens and caches them until shortly
before expiry.
...
# Reuse the same OAuth token file already used by Agent Platform:
