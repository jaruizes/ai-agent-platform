...
type View='dashboard'|'executions'|'approvals'|'agents'|'skills'|'prompts'|'tools'|'mcp'|'knowledge'|'runtime';
...
export class AppComponent implements OnInit,OnDestroy {
  ...
  async refreshAll(){await this.run(async()=>{const [o,r,e,a,ag,sk,pr,to,mc,kb]=await Promise.all([this.api.overview(),this.api.runtime(),this.api.executions(),this.api.approvals(),this.api.agents(),this.api.skills(),this.api.prompts(),this.api.tools(),this.api.mcpServers(),this.api.knowledgeBases()]);this.overview.set(o);this.runtime.set(r);this.executions.set(e);this.approvals.set(a);this.agents.set(ag);this.skills.set(sk);this.prompts.set(pr);this.tools.set(to);this.mcpServers.set(mc);this.knowledgeBases.set(kb);});}
  ...
  setView(v:View){this.view.set(v);if(v==='runtime')this.loadRuntime();}
  ...
  async openExecution(id:string,show=true){try{const [d,o]=await Promise.all([this.api.execution(id),this.api.orchestration(id)]);this.selectedExecution.set(d);this.orchestration.set(o);if(show)this.modal.set('execution');}catch(e:any){this.error.set(this.message(e));}}
  ...
  openCreateExecution(){this.draft={name:'control-plane-request',intent:'',input:'{}',context:'{}',instructions:''};this.modal.set('create-execution');}
  async saveExecution(){await this.run(async()=>{const body={correlationId:crypto.randomUUID(),command:{name:this.draft.name||null,intent:this.draft.intent,input:JSON.parse(this.draft.input||'{}'),context:JSON.parse(this.draft.context||'{}'),instructions:(this.draft.instructions||'').split('\n').filter(Boolean)}};const r=await this.api.createExecution(body);this.closeModal();await this.refreshAll();await this.openExecution(r.executionId);});}
